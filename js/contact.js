window.onload=function(){
    ...
    if(document.getElementById("contact")!=null){
        document.getElementById("contact").onsubmit=function(){
            ...
            //获取性别选项值
            for(iCv=0;iCv<oSex.clientHeight;iCv++){
            ...
            var sInterset="";//获得爱好选项值
            var oInterest=document.getElementsByName("interest");
            for(iCv=0;iCv<oInterest.length;iCv++){
                if(oInterest.item(iCv).checked){
                    sInterset+=oInterest.item(iCv).value;
                }
            }
            document.getElementById("message").style.visibility="visible";
            document.getElementById("submitmessage").innerHTML="<strong> 姓名："+"<br>性别："+sSex+"<br>年龄范围："+sAge+"<bar>爱好:"+sInterset+"<br>电子邮件："+sDzyj+"<br>固定电话"+sTelephone+"<br>公司"+sCompany+"</strong>"
            return false;
        }
    }
}